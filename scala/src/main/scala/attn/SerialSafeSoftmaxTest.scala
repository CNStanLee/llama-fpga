...
class SerialSafeSoftmaxTest(
  ...
  val io = new Bundle {
    ...
    val seqLen = in Bits(addrWidth bits)
  }
  ...
  val inCntOvf = inCnt === io.seqLen.asUInt
  when(input.valid) {
    inCnt := inCnt + 1
    when(inCntOvf) {
      inCnt.clearAll()
  ...
  val accInCntOvf = accInCnt === io.seqLen.asUInt
  when(accIn.valid) {
    accInCnt := accInCnt + 1
    when(accInCntOvf) {
      accInCnt.clearAll()
  ...
  val outCntOvf = outCnt === io.seqLen.asUInt
  ...
  val subCntOvf = subCnt === io.seqLen.asUInt
...
object SerialSafeSoftmaxTest extends App{
  SpinalVerilog(new SerialSafeSoftmaxTest(
    width = 16,
    maxSeqLen = 1024,
