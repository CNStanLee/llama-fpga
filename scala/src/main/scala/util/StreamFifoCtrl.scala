...
class StreamFifoCtrl[T <: Data](val dataType: HardType[T],
                                val depth: Int,
  ...
  val logic = (depth > 1) generate new Area {
    val ptr = new Area {
      ...
      push.addAttribute("max_fanout", 100)
      pop.addAttribute("max_fanout", 100)
...
object StreamFifoCtrl extends App {
  SpinalVerilog(new StreamFifoCtrl(UInt(32 bits), 1024))
}
