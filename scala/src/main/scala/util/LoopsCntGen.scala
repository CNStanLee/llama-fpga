...
object LoopsCntGen {
  ...
  def wireOvf(bound: List[UInt], enable: Bool) = {
  ...
  def regOvf(bound: List[UInt], enable: Bool) = {
    require(bound.nonEmpty)
    val cnt = bound.map(b => UInt(b.getWidth bits) setAsReg() init 0)
...
class LoopsCntGen(width: List[Int]) extends Component {
  val io = new Bundle {
    val enable = in Bool()
  ...
  noIoPrefix()
  val (cnt, cntOvf) = LoopsCntGen.wireOvf(io.bound.toList, io.enable)
  io.cnt := Vec(cnt)
