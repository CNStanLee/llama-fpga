...
class StreamCycleFifoCtrl[T <: Data](dataType: HardType[T], depth: Int, reuseWidth: Int = 8) extends Component {
  require(depth > 1)
  val io = new Bundle {
  ...
  val rdPort = master(MemRdPort(dataType(), depth))
  val wrPort = master(Flow(MemWrPort(dataType(), depth)))
  ...
  rdPort.cmd.valid := popPre.fire
  rdPort.cmd.payload := popPtr
  ...
  wrPort.valid := pushing
  wrPort.payload.address := pushPtr.value
...
object StreamCycleFifoCtrl extends App {
  SpinalVerilog(new StreamCycleFifoCtrl(UInt(8 bits), 1024))
}