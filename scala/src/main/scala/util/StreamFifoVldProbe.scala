...
class StreamFifoVldProbe[T <: Data](val dataType: HardType[T],
                                    val depth: Int,
  ...
  val logic = (depth > 1) generate new Area {
    ...
    val ptr = new Area {
      ...
      val wentUp = RegNextWhen(doPush, doPush =/= doPop) init (False)
    ...
    val pop = new Area {
      ...
      val sync = !withAsyncRead generate new Area {
        ...
        val rValid = Bool().setAsReg().init(False)
        val rValidNext = Bool()
        rValid := rValidNext
        rValidNext := rValid
        when(addressGen.ready) {
          rValidNext := !ptr.empty
        }

        io.popVldNext := rValidNext
