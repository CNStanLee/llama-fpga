...
class SerialRoPE(
  ...
  val (ropeTgM, ropeErr) = TagMap(io.input, tagMap)
  val ropeIn = ropeTgM.m2sPipe()
...
object SerialRoPE extends App {
  val cfg = SpinalConfig(inlineRom = true)
  cfg.generateVerilog(new SerialRoPE(128, 1 << 14, 1, List((1, 2), (3, 4)), util.fp16mul6.mul, util.fp32mul8.mul, util.fp16add8.add, util.fp32toint32d6.to, util.fp32int16d4.from))
}
