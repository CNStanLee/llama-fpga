...
class CosSinGen32(
  ...
  val io = new Bundle {
    ...
    val sinCos = master(Stream(Bits(64 bits)))
  }
  ...
  val table = readIntListFromFile("data/sin4096quarter32.txt")
  val rom = Mem(Bits(32 bits), initialContent = table.map(x => B(x, 32 bits)))
...
object CosSinGen32 extends App {
  val cfg = SpinalConfig(inlineRom = true)
  cfg.generateVerilog(new CosSinGen32(1 << 14))
}