...
#define model_size_ps 2012725248
#define model_size_pl 2012725248
#define EDGELLM_BASE_ADDR 0x0500000000

u64 *region_pl=0x0400000000;
char region_ps[model_size_ps] __attribute__((aligned (8192)));
...
//	printf("%s\n", prompt);
    ...
    printf("PS Region Addr: 0x%lx\n", (u64)(region_ps));

    XTime_GetTime(&tCur);
    response = f_open(&fil, "llmps.bin", FA_OPEN_EXISTING|FA_READ);
    printf("%d ", response);
    response = f_read(&fil, region_ps, model_size_ps, &wr_tot);
    printf("%d ", response);
    response = f_close(&fil);
    printf("%d\n", response);
    XTime_GetTime(&tEnd);
    tUsed = ((tEnd - tCur) * 1000000) / (COUNTS_PER_SECOND);
    printf("Finish First Bank, %d %d\n", model_size_ps, wr_tot);
    printf("Elapsed %ld us\n", tUsed);

    XTime_GetTime(&tCur);
    response = f_open(&fil, "llmpl.bin", FA_OPEN_EXISTING|FA_READ);
    printf("%d ", response);
    response = f_read(&fil, region_pl, model_size_pl, &wr_tot);
    printf("%d ", response);
    response = f_close(&fil);
    printf("%d\n", response);
    XTime_GetTime(&tEnd);
    tUsed = ((tEnd - tCur) * 1000000) / (COUNTS_PER_SECOND);
    printf("Finish First Bank, %d %d\n", model_size_pl, wr_tot);
    printf("Elapsed %ld us\n", tUsed);
    ...
    for(int t=0;t<NUM_TURN;t++){
    	...
    	sleep(1);
//    	Xil_Out32(EDGELLM_BASE_ADDR+0xc4, 4);
//        Xil_Out32(EDGELLM_BASE_ADDR+0xc8, 417);
